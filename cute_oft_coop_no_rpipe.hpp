                ...
                half       *C, CLayout layout_c, WarpLayout warp_layout, Pipeline pipeline)
{
    ...
    Tensor tCrA   = thr_mma1.make_fragment_A(tCsA(_, _, _, 0, 0)); // (MMA, MMA_M, MMA_K)
    Tensor tCrR   = thr_mma1.make_fragment_B(tCsR(_, _, _, 0, 0, 0)); // (MMA, MMA_N, MMA_K)
    ...
    Tensor tCrB   = thr_mma2.make_fragment_B(tCsB(_, _, _, 0, 0, 0)); // (MMA, MMA_N, MMA_K)
    ...
    while (k_tile_count > -(K_PIPE_MAX - 1)) {
        ...
        for (int j = 0; j < K_BLOCK_MAX; ++j) {
            ...
            CUTE_UNROLL
            for (int g = 0; g < GROUP_PER_WARP; ++g) {
                copy(s2r_atom_R{}, tXsR_p(_,_,_,g,j), tXrR);
                clear(tCrAR1); // clear the accumulator for storing AR
                gemm(single_warp_mma1, tCrA, tCrR, tCrAR1);
                ...
                copy(s2r_atom_B{}, tXsB_p(_,_,_,g,j), tXrB);
                gemm(single_warp_mma2, tCrAR2, tCrB, tCrC(_,_,_,g));
            }
        }
        --k_tile_count;
        ++k_tile_next;
